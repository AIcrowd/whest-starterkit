...
------------
  ...
  [OK]    whest install mode     tool-installed · 0.15.0
...
| **BLAS thread pool** | `threadpoolctl` finds a BLAS pool (OpenBLAS, MKL, Accelerate). When absent, numpy falls back to a less-optimized path; FLOPs counts are unaffected, but wall-clock time may be higher and `--max-threads` has no effect. | Install a BLAS-backed numpy (the default `pip install numpy` usually has one). On macOS Apple Silicon, `pip install numpy` ships Accelerate. The warning is harmless for your FLOP count, which is analytical — but wall time is not cosmetic: each MLP has a 120 s wall-clock cap and a 400 ms residual cap, and busting either one zeroes that MLP's predictions. See [Is scoring hardware-dependent?](../troubleshooting/faq.md#is-scoring-hardware-dependent). |

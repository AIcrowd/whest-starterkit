...
```bash
whest dataset bake --output ./my-eval --n-mlps 10 --n-samples 10000 --width 1024 --depth 16
...
| `--flop-budget N` | Matching the grader, or deliberately not matching it. Phase 2 grades each MLP against `B_m = 2,199,023,255,552` FLOPs (2^41), and what the budget caps is `C_m = F_m` — your analytical FLOP count, nothing else — so pass `--flop-budget 2199023255552` for a run that mirrors the grader. (Phase 1 graded at `2.72e11`; the `v1-warmup` round at `6.8e10`.) Bump it to `1e13` to confirm an algorithm idea isn't budget-starved before you optimize for budget. The value you were actually scored under is `run_config.flop_budget` in the run's own report. |
...
| `--wall-time-limit S` | Cap each `predict()` call's wall time. Useful when local debugging hangs on a numerical edge case. The grader's Phase 2 cap is **120 s per MLP**; confirm the value you were scored under in `run_config.wall_time_limit_s` in your own run report rather than copying a number out of the docs. |
| `--residual-wall-time-limit S` | Cap time spent outside flopscope ops (Python plumbing, loops, control flow). Phase 2 enforces a hard **400 ms** residual per MLP — exceed it and that MLP's prediction is replaced with zeros — so `--residual-wall-time-limit 0.4` is the setting that reproduces the grader. Residual time is not priced into your score any more, and it is not a place to compute: meaningful work outside flopscope ops is disqualifiable. |

...
matmuls. `fnp.zeros()`/`fnp.ones()`/`fnp.eye()`/RNG draws all default to float64
...
**Check what dtype `mlp.weights` actually arrives as, and do not assume.** The
evaluation datasets store weights as float32, but a reader that hands rows back
as Python lists will silently give you float64, because Python floats are
doubles. whestbench pins the weights to float32 from the release that follows
0.15.0; **on 0.15.0, which this kit currently resolves, they arrive as float64**.

That matters because it doubles everything downstream: at the competition shape,
`03_covariance_propagation` costs 51,709,240,799 FLOPs against float32 weights
and 103,400,153,536 against float64 — 2.351% of `B_m` versus 4.702%. One line
tells you which world you are in:

```python
print(mlp.weights[0].dtype)
```

If it reports `float64`, cast once per layer — `w = w.astype(fnp.float32)` bills
2,097,152 FLOPs at width 1024 and saves far more than that on the matmul or
einsum that follows.

Either way, **seed your own state explicitly.** One `fnp.zeros(width)` with no
`dtype=` is float64, and NumPy promotion drags every matmul against it onto the
2× rate no matter what the weights are.
...
| Example | float64 state | float32 state | Saving | Final-layer MSE |
|---|---:|---:|---:|---|
...
`.astype()` bills 1 FLOP per element — 2,048 FLOPs per call at width 1024,
against the millions it protects. Check `ctx.summary()` if you suspect a
promotion: a `float64` row where you expected `float32` is the tell.
...
- **Measure it:** `uv run whest run --estimator estimator.py --profile
  --residual-wall-time-limit 0.4`, then read `residual_wall_time_s` per MLP.
  Anything approaching 400 ms locally is a failure waiting to happen on
  hardware you have not seen.

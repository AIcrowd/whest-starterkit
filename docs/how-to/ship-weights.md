...
them into a typed object.) For the save call itself, do it offline with plain `numpy` (`np.savez`): inside `predict()`, `fnp.save`/`fnp.savez` bill an egress cost since flopscope 0.9 (elements × dtype rate, plus headers), while `fnp.load` stays free.

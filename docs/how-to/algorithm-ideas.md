...
**FLOP cost:** O(depth x width^2) — matrix-vector products per layer. For width=256, depth=32: ~20M FLOPs (the warmup 256×8 round was ~2.7M; see the worked walkthrough in [Manage Your FLOP Budget](./manage-flop-budget.md)).
...
**FLOP cost:** O(depth x width^3) — matrix-matrix products per layer. For width=256, depth=32: ~3.2B FLOPs (the warmup 256×8 round was ~404M). Much more expensive.

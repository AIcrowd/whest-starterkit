...
boundary, not per-MLP amnesia. It catches:
...
- Values that do not survive the JSON hop (predictions are serialized and re-read as
  float32)
- **Runaway memory, on Linux** — the worker sets an 8 GB `RLIMIT_AS` there, which is what
  ...
  swallowed unless the worker also dies) and the cap is not applied, so a Mac Stage 4
  pass is **not** proof you fit in 8 GB.

It does **not** reset globals or RNG between MLPs. For that, reset per-MLP state at the
top of `predict()`, or loop `whest run --n-mlps 1 --seed k`.

(The `memory_limit_mb=8192 is advisory in --runner local` notice Stage 3 prints on every
run says the same thing from the other side: the 8 GB solution-process limit is recorded
but not enforced in-process. It is not a warning about your estimator.)
...
If Stage 4 is **worse** than Stage 3, the culprits that can actually differ between the
two runners are:
...
3. **Memory** — on Linux the worker caps itself at 8 GB via `RLIMIT_AS`, so a Stage 3 run
   that quietly used 12 GB is killed there. whestbench replaces the dead worker (re-running
...
4. **Something that does not survive the pipe** — predictions cross as JSON and are re-read
   ...
   `(depth, width)` and nothing exotic.

State bleed between MLPs is *not* on that list: one worker serves the suite, so a global
...
resolves to the directory holding your estimator file) and is where your shipped files
land.

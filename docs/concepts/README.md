...
| [scoring-model.md](scoring-model.md) | How the leaderboard score is computed: ASCII pipeline diagram, explicit equation block for `adjusted_final_layer_score` and `all_layers_mse`, what happens when the FLOP budget or the 400 ms residual cap is blown, and cost/accuracy calibration tables from the bundled examples. |

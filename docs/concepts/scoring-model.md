...
The table below shows the **raw-MSE diagnostics** from the bundled example estimators run against the **public release dataset** — [`arc-whestbench-public-2026`](https://huggingface.co/datasets/aicrowd/arc-whestbench-public-2026), `mini` split (100 MLPs, N=1e9 baked ground truth) — at the 6.8e10 FLOP budget. **⚠️ These figures were measured on the warmup 256×8 round; the Phase 1 dataset is 256×32 (a deeper network), so the MSE and FLOP magnitudes below will change and are pending a 256×32 re-profile.** These are the unscaled `final_layer_mse` / `all_layers_mse` values; the leaderboard `adjusted_final_layer_score` multiplies `final_layer_mse` by `max(0.1, C_m / budget)` (≤ 1.0). Every bundled example spends well under 1% of the budget, so each one bottoms out at the **0.1 floor** — its ranked score is exactly `final_layer_mse ÷ 10`. Use these as calibration points for your own estimator.
